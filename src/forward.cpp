...
void CPU::forward()
{
    ...
    if (D_icode == call || D_icode == jxx)
        d_valA = D_valP;
    else if (d_srcA == E_dstE && d_srcA != no_reg) // ALU输出
        d_valA = e_valE;
    else if (d_srcA == M_dstM && d_srcA != no_reg) // 存储器输出
        d_valA = m_valM;
    else if (d_srcA == M_dstE &&
             d_srcA != no_reg) // 访存阶段对端口E未进行的写
        d_valA = M_valE;
    else if (d_srcA == W_dstM &&
             d_srcA != no_reg) // 写回阶段对端口M未进行的写
        d_valA = W_valM;
    else if (d_srcA == W_dstE &&
             d_srcA != no_reg) // 写回阶段对端口E未进行的写
        d_valA = W_valE;
