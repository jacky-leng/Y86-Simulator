    ...
    while (getline(inputFile, line)) {
        ...
        if (val_str.size() % 2 != 0) {
            throw std::invalid_argument(
                "Hex string should have an even length.");
        }
...
mem_value_64_t memory::read(addr address) {
    ...
    if (memory_map.find(addr_block) == memory_map.end() ||
        ((offset != 0) && memory_map.find(addr_block + 8) == memory_map.end()))
        throw runtime_error("Invalid memory address\n" +
                            std::to_string(address));

    auto firstPart = memory_map[addr_block].rdata;

    firstPart.insert(firstPart.end(), memory_map[addr_block + 8].rdata.begin(),
                     memory_map[addr_block + 8].rdata.end());
    value_t result = 0;
    for (int i = 0; i < BYTE_SIZE; i++) {
        result |= static_cast<value_t>(firstPart[offset + i])
                  << (i * BYTE_SIZE);
    }
    return result;
}
